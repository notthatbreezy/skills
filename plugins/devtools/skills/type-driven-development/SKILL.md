...
- `references/copilot-instructions.md` contains the concise instruction block used when adapting
  these guardrails to repository or user instructions.
- `references/paw-type-safety-specialist.md` contains the PAW specialist profile used when a local
  Society-of-Thought review needs an explicit type-safety reviewer.

Copilot plugins install skills and custom agents natively. They do not write arbitrary user
instruction files or PAW persona directories, so these files remain references rather than hidden
